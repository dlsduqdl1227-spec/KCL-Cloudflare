...
async function upsertOperatorAccount(env, payload, actorArg) {
  ...
  // 같은 엑셀을 다시 업로드해도 중복 계정이 쌓이지 않도록 이름+연락처+권한대회 기준으로 갱신합니다.
  if (!id) {
  ...
  if (!id) {
    const existing = await env.DB.prepare(`SELECT id FROM operators WHERE name=? AND phone=? ORDER BY id LIMIT 1`)
      .bind(data.name, data.phone).first();
    if (existing && existing.id) id = Number(existing.id);
  }
