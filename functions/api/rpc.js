...
async function clearOperators(env, competitionCode, actorArg) {
  ...
  const code = safeStr(competitionCode).toUpperCase();
  if (!code || code === 'ALL') return { success: false, message: '초기화할 대회를 하나 선택하세요.' };
  const rs = await env.DB.prepare('SELECT * FROM operators ORDER BY id').all();
  let deleted = 0, updated = 0, skippedAdmin = 0;
  ...
  return { success: true, message: `${code} 운영계정 초기화 완료: 삭제 ${deleted}건, 권한 수정 ${updated}건. 관리자 계정은 제외했습니다.` };
}
...
function canonicalScoreForPayload_(code, payload, rowIndex=0) {
  ...
  if (code === 'KBC') {
    const p = firstNumberFromKeys_(extra, ['Presentation & Service(프레젠테이션과 서비스 전문성)']) || 0;
  ...
  if (code === 'MOB') {
    const tech = sumFromKeys_(extra, ['Pre-Service Station(시연 전 작업대)','Service Station(시연 중 작업대)','Post-Service Station(시연 후 작업대)','Signature Technical Pre-Service Station(창작음료 시연 전 작업대)','Signature Technical Service Station(창작음료 시연 중 작업대)','Signature Technical Ingredient Use(부재료 사용의 적절함)','Signature Technical Post-Service Station(창작음료 시연 후 작업대)']) || 0;
    const sensory = sumFromKeys_(extra, ['Sweetness(스윗니스)','Flavor(플레이버)','Balance(균형)','Clean Cup(클린컵)','Mouthfeel(질감)','Professionalism(시연 전문성)']) || 0;
    const creative = sumFromKeys_(extra, ['Creative Form & Usability(형태와 용이성)','Creative Flavor(창작 향미)','Creative Balance(균형)','Creative Mouthfeel(질감)','Creative Professionalism(전문성과 독창성)']) || 0;
    const hasMob = (tech + sensory + creative) > 0 || firstNumberFromKeys_(extra, ['시간감점']) !== null;
    if (hasMob) return roundScoreValue_(Math.max(0, tech + sensory + creative - (firstNumberFromKeys_(extra, ['시간감점','Time Penalty']) || 0)));
    return fallback();
  }
  if (code === 'KCAC') {
    ...
    payloadRows.forEach((row, idx) => {
      ...
      let fin = firstNumberFromKeys_(e, ['최종점수','총점','Total Score','Total']);
      if (fin === null) {
        const sub = firstNumberFromKeys_(e, ['소계','Subtotal']);
        const penalty = firstNumberFromKeys_(e, ['감점','Penalty']) || 0;
        if (sub !== null) fin = Math.max(0, sub - penalty);
      }
      if (fin !== null) { total += fin; found = true; }
    });
...
function rowToReviewItem(r, code, headers, fallbackRound) {
  ...
  const computedTotal = canonicalScoreForPayload_(code, payload, 0);
...
async function getFinalReport(env, competitionCode, actorArg) {
  ...
  return {
    ...
    generatedAt: nowIso(),
    tieBreakRule: tieRuleLabel_(code, ''),
...
async function submitScores(env, payload, signature, request = null) {
  ...
  for (const rawPayload of payloads) {
    ...
    await env.DB.prepare(`INSERT INTO scores (submitted_at, competition_code, round, judge_name, team, role, mode, unit, participant_name, total_score, disqualified, disqualification_reason, review_status, payload_json, signature_data)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '미검수', ?, ?)`)
      .bind(nowIso(), x.code, x.round, x.judgeName, x.team, x.role, x.mode, x.unit, x.participantName, x.total, boolInt(x.disqualified), x.dqReason, payloadJson, signature || '').run();
    inserted++;
  }
...
async function getReviewList(env, competitionCode, actorArg) {
  ...
  let list = raw.map(r => rowToReviewItem(r, code, headers, cfg && cfg.current_round));
...
function mobRoleCategoryServer_(roleText) {
  const r = safeStr(roleText).toLowerCase();
  const hasTech = /테크|기술|technical|tech/.test(r);
  const hasSens = /센서|감각|sensory|sensor/.test(r);
...
function mobScoreObjectFromItem_(item) {
  ...
  return {
    ...
    total: Number(item.totalScore ?? item['총점'] ?? item['최종점수'] ?? 0) || 0,
    ...
    pre: n(['Pre-Service Station(시연 전 작업대)','Pre-Service Station']),
    service: n(['Service Station(시연 중 작업대)','Service Station']),
    post: n(['Post-Service Station(시연 후 작업대)','Post-Service Station']),
    signatureTechPre: n(['Signature Technical Pre-Service Station(창작음료 시연 전 작업대)','Signature Technical Pre-Service Station']),
    signatureTechService: n(['Signature Technical Service Station(창작음료 시연 중 작업대)','Signature Technical Service Station']),
    signatureIngredientUse: n(['Signature Technical Ingredient Use(부재료 사용의 적절함)','Signature Technical Ingredient Use']),
    signatureTechPost: n(['Signature Technical Post-Service Station(창작음료 시연 후 작업대)','Signature Technical Post-Service Station']),
    sweetness: n(['Sweetness(스윗니스)','Sweetness']),
    flavor: n(['Flavor(플레이버)','Flavor']),
    balance: n(['Balance(균형)','Balance']),
    cleanCup: n(['Clean Cup(클린컵)','Clean Cup']),
    mouthfeel: n(['Mouthfeel(질감)','Mouthfeel']),
    professionalism: n(['Professionalism(시연 전문성)','Professionalism']),
    creativeForm: n(['Creative Form & Usability(형태와 용이성)','Form & Usability(형태와 용이성)','Creative Form']),
    creativeFlavor: n(['Creative Flavor(창작 향미)','Creative Flavor']),
    creativeBalance: n(['Creative Balance(균형)','Creative Balance']),
    creativeMouthfeel: n(['Creative Mouthfeel(질감)','Creative Mouthfeel']),
    creativeProfessionalism: n(['Creative Professionalism(전문성과 독창성)','Professionalism & Originality(전문성과 독창성)','Creative Professionalism'])
  };
}
...
async function getMobCalibrationParticipantNumbers(env, requestedTeam, roleText, actorArg) {
  ...
  return Array.from(by.values()).sort((a,b) => Number(a.checked) - Number(b.checked) || safeStr(a.participantNo).localeCompare(safeStr(b.participantNo), 'ko'));
}
...
async function markMobCalibrationChecked(env, participantNo, requestedTeam, checkerName, roleText, actorArg) {
  ...
  return { success:true, message:'확인 처리되었습니다.', participantNo:no, checkedAt:payload.checkedAt };
}
...
async function saveIkrcSeedToCupMatch(env, match, actorArg) {
  ...
  const payload = { matchNo, participantA:m.participantA || {}, participantB:m.participantB || {}, memo:safeStr(m.memo), status:safeStr(m.status || '오픈'), savedAt:nowIso(), savedBy:safeStr(auth.actor && auth.actor.name) };
  await env.DB.prepare('INSERT OR REPLACE INTO sessions (token, kind, payload_json, expires_at, created_at) VALUES (?, ?, ?, ?, ?)')
...
async function updateIkrcSeedToCupResult(env, target, bonus, memo, actorArg) {
  ...
  const payload = { targetType, targetValue, bonus:Math.max(0, Math.min(3, Math.round(Number(bonus) || 0))), memo:safeStr(memo), savedAt:nowIso(), savedBy:safeStr(auth.actor && auth.actor.name) };
  await env.DB.prepare('INSERT OR REPLACE INTO sessions (token, kind, payload_json, expires_at, created_at) VALUES (?, ?, ?, ?, ?)')
...
function mobComponentsFromItem_(item) {
  const tech = itemScoreSum_(item, MOB_TECH_KEYS_);
  const sensory = itemScoreSum_(item, MOB_SENS_KEYS_);
  const creative = itemScoreSum_(item, MOB_CREATIVE_KEYS_);
  const penalty = itemScore_(item, ['시간감점','Time Penalty']);
  const role = safeStr(item && (item['역할'] || item.role));
  const techRole = /테크|기술|technical|tech/i.test(role) && !/센서|감각|sensory|sensor/i.test(role);
  const sensoryRole = /센서|감각|sensory|sensor/i.test(role) && !/테크|기술|technical|tech/i.test(role);
  const hasTech = techRole ? true : (sensoryRole ? false : tech > 0);
  const hasSensory = sensoryRole ? true : (techRole ? false : sensory > 0);
  const hasCreative = sensoryRole ? creative > 0 : (techRole ? false : creative > 0);
  return { tech, sensory, creative, penalty, hasTech, hasSensory, hasCreative, time: itemEndTimeSeconds_(item) };
}
...
async function buildRankingData_(env, competitionCode) {
  ...
  const converted = raw.map(r => {
    let item = rowToReviewItem(r, code, headers, cfg && cfg.current_round);
...
async function getRanking(env, competitionCode, actorArg) {
  ...
  if (!auth.ok) return auth.res;
  const data = await buildRankingData_(env, code);
  return { success: true, compCode: code, compName: data.cfg ? data.cfg.name : code, currentRound: data.cfg ? data.cfg.current_round : '', unitLabel: code === 'KTCC' ? '팀번호' : '참가자번호', ranking: data.ranking, tieBreakRule: tieRuleLabel_(code, '') };
}
