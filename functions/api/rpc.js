...
async function dispatch(action, args, env) {
  const handlers = {
    ...
    createDebriefPdfFromPayload: () => ({ success: false, message: 'V7에서는 브라우저 인쇄 기능으로 PDF 저장을 지원합니다. 화면의 PDF 저장 버튼을 다시 눌러 저장하세요.' }),
    createRankingDetailPdf: () => ({ success: false, message: 'V7에서는 브라우저 인쇄 기능으로 PDF 저장을 지원합니다. 화면의 PDF 저장 버튼을 다시 눌러 저장하세요.' }),
...
function participantPayloadFromRow_(raw, defaultCode='') {
  const code = safeStr(pickByAliases_(raw, ['대회코드','competition_code','competitionCode','code'], defaultCode)).toUpperCase();
  ...
  const teamNo = safeStr(pickByAliases_(raw, ['팀번호','team_no','teamNo']));
  const uniqueNo = safeStr(pickByAliases_(raw, ['고유번호','unique_no','uniqueNo','참가자번호','선수번호','번호']));
  const prelim = safeStr(pickByAliases_(raw, ['예선컵번호','prelim_cup_no','prelimCupNo','예선번호']));
  const main = safeStr(pickByAliases_(raw, ['본선컵번호','main_cup_no','mainCupNo','본선번호']));
  const final = safeStr(pickByAliases_(raw, ['결선컵번호','final_cup_no','finalCupNo','결선번호']));
  const cupNo = safeStr(pickByAliases_(raw, ['컵번호','cup_no','cupNo','출품번호']));
  const sampleNo = safeStr(pickByAliases_(raw, ['샘플번호','sample_no','sampleNo']));
...
function operatorPayloadFromRow_(raw, defaultCode='') {
  const access = safeStr(pickByAliases_(raw, ['권한대회','access','대회코드','competition_code','code'], defaultCode)).toUpperCase();
  return {
    accountType: safeStr(pickByAliases_(raw, ['계정유형','account_type','accountType','type'], 'JUDGE')).toUpperCase(),
...
async function getRegistryData(env, competitionCode, actorArg) {
  const actor = await getActor(env, actorArg);
  if (!hasAdmin(actor)) return { success: false, message: '등록 관리 권한이 없습니다.' };
  ...
  return { success: true, configs: cfg.configs, participants: parts.participants || [], operators: ops.results || [], templates: getRegistrationTemplates().templates };
}
async function listParticipants(env, competitionCode, actorArg) {
  const actor = await getActor(env, actorArg);
  if (!hasAdmin(actor)) return { success: false, message: '선수 등록 조회 권한이 없습니다.' };
  const code = safeStr(competitionCode).toUpperCase();
  const rs = code && code !== 'ALL'
    ? await env.DB.prepare('SELECT * FROM participants WHERE competition_code=? ORDER BY id').bind(code).all()
    : await env.DB.prepare('SELECT * FROM participants ORDER BY competition_code, id').all();
...
async function upsertParticipant(env, payload, actorArg) {
  const actor = await getActor(env, actorArg);
  if (!hasAdmin(actor)) return { success: false, message: '선수 등록 권한이 없습니다.' };
  ...
  const id = Number(payload && (payload.rowIndex || payload.id));
...
async function deleteParticipant(env, rowIndex, actorArg) {
  const actor = await getActor(env, actorArg);
  if (!hasAdmin(actor)) return { success: false, message: '선수 삭제 권한이 없습니다.' };
...
async function clearParticipants(env, competitionCode, actorArg) {
  const actor = await getActor(env, actorArg);
  if (!hasAdmin(actor)) return { success: false, message: '초기화 권한이 없습니다.' };
  const code = safeStr(competitionCode).toUpperCase();
...
async function importParticipants(env, payload, actorArg) {
  const actor = await getActor(env, actorArg);
  if (!hasAdmin(actor)) return { success: false, message: '선수 일괄 등록 권한이 없습니다.' };
...
function tieInfoForItem_(code, item, round) {
  ...
  if (code === 'KCR') return { sweetness:itemScore_(item, ['Sweetness(스윗니스) ×2','Sweetness(단맛) ×2','스윗니스','Sweetness','Sweetness(스윗니스)']), overall:itemScore_(item, ['Overall(주관적 종합평가)','Overall','오버롤']) };
  if (code === 'IKRC') return { flavor:itemScore_(item, ['Flavor(플레이버) ×3','Flavor(플레이버)','Flavor']), sweetness:itemScore_(item, ['Sweetness(스윗니스) ×2','Sweetness(스윗니스)','Sweetness']), mouthfeel:itemScore_(item, ['Mouthfeel(마우스필) ×2','Mouthfeel(마우스필)','Mouthfeel']) };
  if (code === 'MOC' || code === 'KTCC') return { time:itemEndTimeSeconds_(item) };
  if (code === 'MOB') {
    return {
      sensory: itemScore_(item, ['Sweetness(스윗니스)','Flavor(플레이버)','Balance(균형)','Clean Cup(클린컵)','Mouthfeel(질감)','Professionalism(시연 전문성)']),
      technical: itemScore_(item, ['Pre-Service Station(시연 전 작업대)','Service Station(시연 중 작업대)','Post-Service Station(시연 후 작업대)']),
      creative: itemScore_(item, ['Creative Form & Usability(형태와 용이성)','Creative Flavor(창작 향미)','Creative Balance(균형)','Creative Mouthfeel(질감)','Creative Professionalism(전문성과 독창성)']),
      time: itemEndTimeSeconds_(item)
    };
  }
  if (code === 'KCAC') {
    if (r === '결선') return { sensory:itemScore_(item, ['결선 Taste Balance(맛의 균형)','결선 Mouthfeel(질감)','결선 Presentation(프레젠테이션)']), design:itemScore_(item, ['결선 Design Completion(디자인 완성도)','결선 Theme Expression(주제 표현력)']), time:itemEndTimeSeconds_(item) };
    return { completion:itemScore_(item, ['예선 Pattern Completion(패턴 완성도)','Pattern Completion(패턴 완성도)']), balance:itemScore_(item, ['예선 Pattern Balance(패턴 균형)','Pattern Balance(패턴 균형)']), time:itemEndTimeSeconds_(item) };
  }
...
async function sendOTP(env, name, phone, competitionCode) {
  ...
  if (safeStr(env.SMS_PROVIDER).toLowerCase() === 'solapi') {
    // TODO: SOLAPI/NCP 문자 발송 연결. 현재는 개발 모드 인증번호 반환.
  }
  return { success: true, message: env.SMS_PROVIDER ? '인증번호를 발송했습니다.' : '개발 모드 인증번호: ' + otp, maskedPhone: maskPhone_(phone), devOtp: env.SMS_PROVIDER ? undefined : otp };
}
