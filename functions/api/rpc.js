...
async function clearScores(env, competitionCode, actorArg) {
  ...
  const code = safeStr(competitionCode).toUpperCase();
  if (!code || code === 'ALL') return { success: false, message: '점수를 초기화할 대회를 하나 선택하세요.' };
  ...
  if (code === 'MOB') {
    const rs = await env.DB.prepare("SELECT token FROM sessions WHERE kind IN ('MOB_CALIBRATION_CHECK') AND payload_json LIKE ?").bind('%"competitionCode":"MOB"%').all();
  ...
  return { success: true, message: code + ' 점수/검수/순위 데이터 초기화 완료: 점수 ' + Number(before && before.n || 0) + '건 삭제' + (sessionDeleted ? ', 보조 검수 데이터 ' + sessionDeleted + '건 삭제' : '') };
}
...
function kcacRowTotalFromExtra_(extra) {
  extra = extra || {};
  const sub = firstNumberFromKeys_(extra, ['소계','Subtotal']);
  ...
  if (sub !== null) return penalty >= 999 ? 0 : roundScoreValue_(Math.max(0, sub - penalty));
...
async function getFinalReport(env, competitionCode, actorArg) {
  ...
  return {
    ...
    generatedAt: nowIso(),
    tieBreakRule: tieRuleLabel_(code, item['라운드'] || item.round || ''),
