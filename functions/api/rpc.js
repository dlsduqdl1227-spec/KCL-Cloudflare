...
async function dispatch(action, args, env) {
  const handlers = {
    ...
    createDebriefPdfFromPayload: () => ({ success: false, message: 'PDF 생성 기능은 다음 단계에서 Cloudflare 기반 PDF 서비스로 연결 예정입니다.' }),
    createRankingDetailPdf: () => ({ success: false, message: 'PDF 생성 기능은 다음 단계에서 Cloudflare 기반 PDF 서비스로 연결 예정입니다.' }),
