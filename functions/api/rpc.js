...
function mobOfficialTotalFromExtra_(extra) {
  ...
  if (hasRaw) {
    const computed = roundScoreValue_(Math.max(0, tech + sensory + creative - penalty));
    // 시간초과 실격처럼 UI가 이미 총평가 반영점수를 0점으로 저장한 경우를 보존한다.
    if (direct !== null && direct <= 0 && computed > 0) return 0;
    return computed;
  }
