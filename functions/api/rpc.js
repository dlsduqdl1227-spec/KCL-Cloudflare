...
function normalizeParticipantImportRows_(rows, defaultCode='') {
  ...
  teamGroups.forEach(groupRows => {
    ...
    })).filter(m => m.name || m.affiliation || m.phone || m.note);
    const merged = { ...first };
    merged['대회코드'] = 'KTCC';
    merged['팀번호'] = teamNo;
    merged['팀명'] = teamName;
...
function getRegistrationTemplates() {
  // 웹 화면의 CSV 예시는 다운로드 엑셀과 같은 원본 공통 양식을 기준으로 제공합니다.
  // 실제 사용 열은 대회/라운드별 정책에서 결정됩니다.
  const participantCsv = [
    '대회코드,참가자번호,선수명,소속,연락처,컵번호,샘플번호,팀번호,팀명,예선컵번호,본선컵번호,결선컵번호,비고',
    'KBC,1,KBC 선수1,카페A,01011112222,,,,,,,,현장 평가',
    'MOC,1,MOC 선수1,카페A,01022223333,1,,,,,,,감별형 대회',
    'MOB,1,MOB 선수1,로스터리A,01033334444,,,,,,,,브루잉 시연',
    'KCR,1,KCR 선수1,로스터리A,01044445555,KCR-P001,,,,KCR-P001,,KCR-F001,블라인드 출품번호',
    'IKRC,1,IKRC 선수1,카페A,01055556666,,A-1,,,A-1,,F-1,샘플번호 중심',
    'KCAC,1,KCAC 선수1,카페A,01066667777,KCAC-P001,,,,KCAC-P001,,KCAC-F001,온라인 예선 블라인드 번호',
    'KTCC,,KTCC 선수1,A카페,01077778888,,,1,팀커핑A,1,,F1,팀 단위 등록'
  ].join('\n');
  const operatorCsv = [
...
async function getSystemStatus(env, actorArg) {
  ...
  return {
    success: true,
    version: '1.0ver-security-stage23-registry-original-template-final',
