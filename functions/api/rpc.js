...
async function getActor(env, actor) {
  ...
  if (!actor.judgeToken) return null;
  const row = await env.DB.prepare('SELECT payload_json FROM sessions WHERE token=? AND expires_at > ?').bind(safeStr(actor.judgeToken), nowIso()).first();
  const payload = row ? parseJson(row.payload_json, null) : null;
  if (!payload) return null;
  return hydrateActorFromOperators_(env, payload);
}
...
async function getSystemStatus(env, actorArg) {
  ...
  return {
    success: true,
    version: '1.0ver-security-stage5',
