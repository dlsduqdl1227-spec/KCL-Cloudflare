...
async function getFinalReport(env, competitionCode, actorArg) {
  ...
  const rows = data.rows.map(item => reportRowOut_(item, data.headers));
  const approvedRows = rows.filter(r => safeStr(r['검수상태']).replace(/\s/g,'') === '검수완료');
  const rawRows = data.rows.map(item => ({
    id: item.rowIndex,
...
async function buildRankingData_(env, competitionCode) {
  ...
  const converted = raw.map(r => {
    const item = rowToReviewItem(r, code, headers, cfg && cfg.current_round);
    ...
    return enrichReviewItemWithParticipant_(item, identity, code);
  });
