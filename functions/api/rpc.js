...
function aggregateRankingGroup_(code, g, round) {
  ...
  if (code === 'MOB') {
    ...
    if (tech.length || sensory.length || creative.length) {
      const sensoryAvg = avgList_(sensory);
      const technicalAvg = avgList_(tech);
      const creativeAvg = avgList_(creative);
      const timePenalty = penalties.length ? Math.max(...penalties) : 0;
      const gross = Math.round((sensoryAvg + technicalAvg + creativeAvg) * 1000) / 1000;
      ...
      return {
        ...
        basis: timePenalty ? '카테고리별 평균 합산 - 시간감점' : '카테고리별 평균 합산',
        tie: { sensory: sensoryAvg, technical: technicalAvg, creative: creativeAvg, timePenalty, gross, penaltyApplied, time: times.length ? Math.min(...times) : 999999 }
      };
    }
  }
