...
async function getActor(env, actor) {
  ...
  if (actor.judgeToken) {
    ...
    return hydrateActorFromOperators_(env, payload || actor);
  }
  return hydrateActorFromOperators_(env, actor);
}
...
function generateCuppingComment(payload) {
  ...
  const body = _tagPhrase_(tags, 'body', '중간 정도의');
  const sweet = _tagPhrase_(tags, 'sweetness', '단맛');
  const tone = _toneByScore_(_avg(items.map(x=>x.score)), 5);
  const strength = _strengthSentence_(items, '가장 안정적으로 드러났고', '선명도와 지속성을 더 정리하면 좋겠습니다');
  const prefix = cupNo ? `${cupNo}번 컵은 ` : '해당 컵은 ';
  const processText = process ? `${process} 프로세스의 개성을 바탕으로 ` : '';
  return _optionSet([
    `${prefix}${processText}${flavor} 인상을 중심으로 첫 향미가 열리고, 후반부에는 ${after}의 흐름이 이어집니다. 산미는 ${acidity} 방향으로 구조를 만들고, 바디는 ${body} 질감을 보태며, 단맛은 ${sweet} 인상으로 균형을 받쳐줍니다. 전체적으로 ${tone} 컵으로 보이며, ${strength}`,
    `${prefix}${flavor}를 중심으로 향미의 첫인상이 형성됩니다. 이후 ${after}, ${acidity}의 산미, ${body} 질감이 차례로 연결되며 컵의 입체감을 만듭니다. ${sweet}이 더 안정적으로 유지될수록 전체 균형과 오버롤의 설득력이 높아질 수 있습니다.`,
    `검수 관점에서 이 컵은 향미의 방향성, 여운의 정리감, 산미와 질감의 균형을 함께 볼 필요가 있습니다. 현재 인상은 ${flavor}, ${after}, ${sweet}이 주요 단서로 확인되며, ${strength}`
  ]);
}
...
function generateKbcComment(payload) {
  ...
  const serviceText = presentation >= 5 ? '서비스와 프레젠테이션 흐름이 안정적으로 구축되었습니다' : presentation >= 3.5 ? '서비스 흐름은 기준을 충족하지만 설명의 밀도와 동선의 정교함을 더할 여지가 있습니다' : '서비스 전달과 프레젠테이션의 안정성에서 보완이 필요합니다';
  const espressoText = espressoAvg >= 5 ? '에스프레소는 맛의 설계, 클린컵, 질감, 플레이버가 비교적 자연스럽게 연결됩니다' : espressoAvg >= 4 ? '에스프레소는 전반적으로 안정적이나 향미의 선명도와 균형을 조금 더 끌어올릴 수 있습니다' : '에스프레소는 추출 안정성, 향미 표현, 질감의 일관성을 함께 보완할 필요가 있습니다';
  const sigText = !isMain ? '' : (sigAvg >= 5 ? '창작음료는 에스프레소와 부재료의 연결성이 분명하고 메뉴 설계 의도가 잘 드러납니다' : sigAvg >= 4 ? '창작음료는 구성은 안정적이나 핵심 향미 포인트를 더 선명하게 만들 수 있습니다' : '창작음료는 의도 전달, 향미 균형, 질감의 연결성을 추가로 다듬을 필요가 있습니다');
  const machineText = machine >= 5 ? '장비 운용과 작업대 관리도 전체 수행의 완성도를 뒷받침합니다' : machine >= 3.5 ? '장비 운용은 기준을 충족하지만 세부 동작과 작업대 정리의 완성도를 높일 여지가 있습니다' : '장비 운용과 작업대 관리 측면의 보완이 필요합니다';
  return _optionSet([
    `${serviceText}. ${espressoText}. ${isMain ? sigText + '. ' : ''}${machineText}. 전체적으로 음료의 맛과 서비스 전달이 하나의 흐름으로 이어지는지가 핵심입니다.`,
    `이번 수행은 서비스 전달, 에스프레소 완성도${isMain ? ', 창작음료 설계' : ''}, 장비 운용의 연결성을 중심으로 평가됩니다. 스마트태그 기준으로는 ${tagSummary}이 확인되며, ${comments.length ? '세부 코멘트에서는 ' + comments.join(' / ') + '가 주요 참고 지점입니다.' : '현장 설명과 컵의 실제 인상을 더 밀접하게 맞추는 것이 중요합니다.'}`,
    `보완 시에는 낮게 기록된 세부 항목을 단순히 따로 보지 않고, 추출 일관성·향미 설명·서비스 동선이 함께 개선되는 방향으로 조정하는 것이 좋습니다. ${isMain ? '창작음료는 콘셉트와 실제 향미가 자연스럽게 연결될수록 설득력이 높아집니다.' : '에스프레소의 선명한 표현과 안정적인 서비스 흐름이 전체 인상을 좌우합니다.'}`
  ]);
}
...
function generateKcacComment(payload) {
  ...
  const strength = _strengthSentence_(scoreItems, '가장 안정적으로 표현되었고', '정확도와 재현성을 더 다듬으면 좋겠습니다');
  if (isSensory) {
    return _optionSet([
      `${label}은 ${milk ? milk + ' 조건에서 ' : ''}맛의 균형, 질감, 프레젠테이션의 연결성을 중심으로 평가되었습니다. 음료의 인상이 분리되지 않고 하나의 경험으로 이어지는지가 핵심이며, ${tagText}이 주요 참고 지점입니다.`,
      `센서리 관점에서는 맛의 균형과 촉감, 전달 방식이 함께 읽힙니다. 전체 인상은 ${_toneByScore_(avg, 5)} 수준으로 확인되며, ${strength}`,
      `보완한다면 맛의 중심축과 질감의 지속성, 설명의 명확성을 함께 정리하는 것이 좋습니다. 컵의 콘셉트와 실제 음용 인상이 가까워질수록 최종 표현력이 더 안정적으로 전달됩니다.`
    ]);
  }
  return _optionSet([
    `${label}은 ${milk ? milk + ' 조건에서 ' : ''}${pattern}의 완성도, 표면 품질, 위치와 비율을 중심으로 평가되었습니다. ${tagText}을 기준으로 볼 때 전체적인 시각 완성도는 ${_toneByScore_(avg, 5)} 편입니다.`,
    `패턴 평가는 중심축, 대칭, 리프 간격, 라인의 선명도와 표면 정리감이 핵심입니다. ${strength}`,
    `보완 시에는 패턴의 대비, 표면 질감, 중심 위치를 더 정교하게 맞추는 것이 중요합니다. 같은 패턴을 반복해도 안정적으로 재현될수록 시각적 완성도와 평가 설득력이 함께 높아집니다.`
  ]);
}
...
function generateMobComment(payload) {
  ...
  const techText = techAvg ? `기술 수행은 ${_toneByScore_(techAvg, 7)} 수준으로, 준비 과정과 서비스 동선이 추출 결과를 뒷받침합니다` : '기술 수행보다는 센서리와 메뉴 설계 중심으로 평가되었습니다';
  const sensText = sensAvg ? `센서리 항목은 ${_toneByScore_(sensAvg, 7)} 흐름을 보이며, 단맛·플레이버·균형·클린컵·질감의 연결성이 컵의 설득력을 만듭니다` : '센서리 항목은 제출 데이터 기준으로 별도 점수 흐름이 확인되지 않았습니다';
  const sigText = isCreative ? (sigAvg ? `창작 요소는 ${_toneByScore_(sigAvg, 7)} 완성도로, 형태와 용이성, 향미, 균형, 전문성의 연결성이 중요하게 작용합니다` : '창작 메뉴는 설계 의도와 실제 향미의 연결성을 중심으로 보완점을 볼 수 있습니다') : '기본 브루잉 메뉴에서는 추출 설계와 서비스 설명의 일관성이 핵심입니다';
  return _optionSet([
    `${menu} 평가는 추출 설계, 서비스 흐름, 향미 표현이 서로 맞물리는지를 중심으로 진행됩니다. ${techText}. ${sensText}.`,
    `스마트태그 기준으로는 ${tagSummary}이 확인됩니다. ${sigText}. ${comments.length ? '세부 코멘트에서는 ' + comments.join(' / ') + '가 주요 참고 지점입니다.' : '컵의 의도와 실제 인상을 더 자연스럽게 연결하는 것이 중요합니다.'}`,
    `보완 시에는 추출 일관성, 향미 균형, 설명의 명확성을 함께 조정하는 것이 좋습니다. 메뉴의 의도와 컵의 실제 인상이 자연스럽게 연결될수록 평가 설득력이 높아집니다.`
  ]);
}
...
function generateIkrcComment(payload) {
  ...
  const strength = _strengthSentence_(items, '가장 안정적으로 드러났고', '로스팅 포인트와 표현 선명도를 더 조정하면 좋겠습니다');
  const prefix = sample ? `Sample ${sample}은 ` : '해당 샘플은 ';
  return _optionSet([
    `${prefix}${flavor} 계열의 향미가 첫인상을 형성하고, ${clean}한 인상이 컵의 완성도를 뒷받침합니다. 단맛은 ${sweet} 방향으로 구조를 만들며, 산미는 ${acidity}, 마우스필은 ${mouthfeel} 특성으로 이어집니다.`,
    `로스팅 결과는 향미의 선명도, 후반부 클린함, 단맛 지속성의 균형을 중심으로 볼 수 있습니다. 향미 강도는 ${intensities.flavor || '-'} 수준으로 기록되며, 전체적으로 ${_toneByScore_(_avg(items.map(x=>x.score)), 10)} 샘플로 평가됩니다.`,
    `${strength} 특히 단맛과 산미, 질감의 연결성이 자연스럽게 맞을수록 로스팅 의도와 컵의 실제 인상이 더 설득력 있게 전달됩니다.`
  ]);
}
