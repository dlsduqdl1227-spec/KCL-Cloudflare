...
function rowToReviewItem(r, code, headers, fallbackRound, payloadRowIndex=0) {
  ...
  if (safeStr(code).toUpperCase() === 'MOB') {
    ...
    if (mobComp.hasRaw) {
      item['감점 전 합산'] = mobComp.gross;
      item['감점 적용 후 점수'] = mobComp.penaltyApplied;
      item['순위 반영점수'] = mobComp.official;
      item['총평가 반영점수'] = mobComp.official;
    }
  }
...
async function getFinalReport(env, competitionCode, actorArg) {
  ...
  const finalItems = data.rows.filter(item => reviewCompletedStatus_(item['검수상태'] || item.status) && shouldCountItemInRanking_(code, item));
...
async function getReviewList(env, competitionCode, actorArg) {
  ...
  const raw = manager ? rawAll : rawAll.filter(r => reviewScoreVisibleToActor_(r, auth.actor, code, false));
...
async function updateReviewRow(env, competitionCode, rowIndex, updates, newStatus, roleText, actorArg) {
  ...
  if (safeStr(current.review_status) === '검수완료' && statusRequested !== '검수완료' && !manager) {
  ...
  const rawForHeaders = manager ? rawAll : rawAll.filter(r => reviewScoreVisibleToActor_(r, actor, code, false));
...
function calibrationJudgeIdentityKey_(item) {
  item = item || {};
  const judge = safeStr(item.judgeName || item['심사위원명'] || item['심사위원'] || item.operatorName || item['운영자명']).replace(/\s+/g,'').toLowerCase();
...
function kbcRawSubtotalFromItem_(item) {
  ...
  const presentation = firstNumberFromKeys_(item, ['Presentation Total','프레젠테이션 총점','프레젠테이션 합산']) || 0;
  const espresso = kbcEspressoTotalFromItem_(item) || 0;
  const signature = kbcSignatureTotalFromItem_(item) || 0;
  const machine = firstNumberFromKeys_(item, ['Machine Total','기계사용 총점','기계운용 총점']) || 0;
  const hasRaw = itemHasAnyScore_(item, [
    'Presentation Total','프레젠테이션 총점','프레젠테이션 합산',
...
function aggregateRankingGroup_(code, g, round) {
  ...
  if (code === 'KCR') {
    const score = avgRowsBy_(activeRows, kcrOfficialScoreFromItem_) || 0;
    return {
      score, total: score, basis: activeRows.some(item => firstNumberFromKeys_(item, ['정규화점수','Normalized Score','공식점수','순위반영점수']) !== null) ? '정규화 총점' : '항목 총점',
      tie: {
        sweetness: avgRowsBy_(activeRows, item => firstNumberFromKeys_(item, ['Sweetness(스윗니스) ×2','Sweetness(단맛) ×2','Sweetness(스윗니스)','Sweetness','스위트니스','스윗니스'])) || 0,
        overall: avgRowsBy_(activeRows, item => firstNumberFromKeys_(item, ['Overall(주관적 종합평가)','Overall','오버롤'])) || 0
      }
    };
  }

  if (code === 'IKRC') {
    const score = avgRowsBy_(activeRows, rankingScoreFromItem_) || 0;
    return {
      score, total: score, basis: '최종 총점' + (activeRows.some(item => firstNumberFromKeys_(item, ['Seed to Cup Bonus','SeedToCup Bonus','Seed to Cup(+3점)','Seed to Cup']) !== null) ? ' + Seed to Cup' : ''),
      tie: {
        flavor: avgRowsBy_(activeRows, item => firstNumberFromKeys_(item, ['Flavor(플레이버) ×3','Flavor(플레이버)','Flavor','플레이버','향미'])) || 0,
        sweetness: avgRowsBy_(activeRows, item => firstNumberFromKeys_(item, ['Sweetness(스윗니스) ×2','Sweetness(단맛)','Sweetness(스윗니스)','Sweetness','스위트니스','스윗니스','단맛'])) || 0,
        mouthfeel: avgRowsBy_(activeRows, item => firstNumberFromKeys_(item, ['Mouthfeel(마우스필) ×2','Mouthfeel(질감)','Mouthfeel(마우스필)','Mouthfeel','마우스필','촉감','질감'])) || 0
      }
    };
  }
...
async function buildRankingData_(env, competitionCode) {
  ...
  Object.keys(byRound).forEach(round => {
    ...
    list.forEach((g, idx) => ranking.push({
      ...
      totalScore: g.rankingScore, avgScore: g.rankingScore, score: g.rankingScore, rankingScore: g.rankingScore,
...
async function getRankingDetail(env, competitionCode, unit, round, actorArg) {
  ...
  if (!auth.ok) return auth.res;
  const data = await buildRankingData_(env, code);
  const countableRows = data.rows.filter(item => shouldCountItemInRanking_(code, item));
