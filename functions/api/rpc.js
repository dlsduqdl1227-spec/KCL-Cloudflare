...
async function getScoreBackupReport(env, competitionCode, actorArg) {
  ...
  const competitionRows = rows.filter(row => row['백업구분'] === '실제대회평가');
  ...
  const categoryCounts = rows.reduce((acc, row) => {
    const category = safeStr(row['백업구분'] || '실제대회평가') || '실제대회평가';
...
function aggregateRankingGroup_(code, g, round) {
  ...
  if (code === 'KCR') {
    ...
    return {
      score, total: score, basis: activeRows.some(item => firstNumberFromKeys_(item, ['정규화점수','Normalized Score','공식점수','순위반영점수']) !== null) ? '정규화/공식점수 평균' : 'KCR 항목 총점 평균',
      tie: {
  ...
  if (code === 'IKRC') {
    ...
    return {
      score, total: score, basis: '센서리 심사위원 평균값' + (activeRows.some(item => firstNumberFromKeys_(item, ['Seed to Cup Bonus','SeedToCup Bonus','Seed to Cup(+3점)','Seed to Cup']) !== null) ? ' + Seed to Cup' : ''),
      tie: {
  ...
  if (code === 'KBC') {
    ...
    return {
      score, total: score, basis: '평가표 총합 평균 - 시간감점',
      tie: {
  ...
  return { score: avg, total: avg, basis: activeRows.length > 1 ? '심사위원 평균점수' : '총점', tie: g.tie };
}
