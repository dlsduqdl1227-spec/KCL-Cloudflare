...
db.seedOperator({account_type:'JUDGE', name:'다른심사위원', phone:'01033334444', access:'KCR,KCAC,MOC,KTCC,IKRC,KBC,MOB', role:'센서리 심사위원', team_group:'B'});
...
let otherJudge = await rpc('judgeLogin', ['다른심사위원','01033334444']);
let lead = await rpc('judgeLogin', ['테스트팀장','01022223333']);
assert.equal(judge.success,true); assert.equal(otherJudge.success,true); assert.equal(lead.success,true);
let ownReviewInitial = await rpc('getReviewList', ['KCR', judge]);
assert.equal(ownReviewInitial.success,true, 'judge may open own review list');
assert.equal(ownReviewInitial.ownOnly,true, 'judge review list is own-only');
assert.equal(ownReviewInitial.list.length,0, 'judge own review list starts empty');
...
let ownReviewAfter = await rpc('getReviewList', ['KCR', judge]); assert.equal(ownReviewAfter.success,true); assert.equal(ownReviewAfter.list.length,2, 'judge sees only own KCR submissions');
let otherReviewAfter = await rpc('getReviewList', ['KCR', otherJudge]); assert.equal(otherReviewAfter.success,true); assert.equal(otherReviewAfter.list.length,0, 'other judge cannot see these submissions');
let deniedOtherApprove = await rpc('updateReviewStatus', ['KCR', 1, '검수완료', '', otherJudge]); assert.equal(deniedOtherApprove.success,false, 'other judge cannot approve someone else submission');
let ownApprove = await rpc('updateReviewStatus', ['KCR', 1, '검수완료', '', judge]); assert.equal(ownApprove.success,true, 'judge can approve own submission');
let ownRollbackDenied = await rpc('updateReviewStatus', ['KCR', 1, '미검수', '', judge]); assert.equal(ownRollbackDenied.success,false, 'judge cannot rollback completed own review');
await approve('KCR',[2], lead);
...
console.log('STAGE18_DIRECT_QA_PASS', JSON.stringify({scores:db.tables.scores.length, kcrTop:reportKcr.ranking[0].unit, kcacImages:review.list[0].mediaCount, mocSignature:detail.rows[0].signatureData?true:false}));
