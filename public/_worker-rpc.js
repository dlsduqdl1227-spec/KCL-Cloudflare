...
async function dispatch(action, args, env, request) {
  const handlers = {
    ...
    liveAuditRun: () => liveAuditRun(env, args[0] || {}, request),
    liveAuditRunStep: () => liveAuditRunStep(env, args[0] || {}, request)
  };
...
async function updateReviewRow(env, competitionCode, rowIndex, updates, newStatus, roleText, actorArg) {
  ...
  const item = rowToReviewItem({...current, payload_json: JSON.stringify(payload)}, code, tmpHeaders, '', targetRowIndex); const inferred = inferScorePayload(payload);
  ...
  const dq = item['실격여부'] === 'Y' || inferred.disqualified || !!current.disqualified; const dqReason = firstNonEmpty([item['실격사유'], inferred.dqReason, current.disqualification_reason]); const status = statusRequested;
...
const LIVE_AUDIT_DEFAULT_TOKEN_ = 'STAGE62-LIVE-AUDIT-5061-7d9f8c2a9b1e';
function liveAuditExpectedToken_(env) {
  return safeStr(env && (env.KCL_LIVE_AUDIT_TOKEN || env.LIVE_AUDIT_TOKEN)) || LIVE_AUDIT_DEFAULT_TOKEN_;
}
function liveAuditCleanSqls_() {
  return [
    [`DELETE FROM scores WHERE judge_name LIKE '__AI_TEST_%' OR participant_name LIKE '__AI_TEST_%' OR unit LIKE 'AI62-%' OR unit LIKE 'AI63-%' OR payload_json LIKE '%__AI_TEST_%'`],
    [`DELETE FROM participants WHERE name LIKE '__AI_TEST_%' OR team_name LIKE '__AI_TEST_%' OR unique_no LIKE 'AI62-%' OR unique_no LIKE 'AI63-%' OR cup_no LIKE 'AI62-%' OR cup_no LIKE 'AI63-%' OR sample_no LIKE 'AI62-%' OR sample_no LIKE 'AI63-%' OR team_no LIKE 'AI62-%' OR team_no LIKE 'AI63-%' OR prelim_cup_no LIKE 'AI62-%' OR prelim_cup_no LIKE 'AI63-%' OR main_cup_no LIKE 'AI62-%' OR main_cup_no LIKE 'AI63-%' OR final_cup_no LIKE 'AI62-%' OR final_cup_no LIKE 'AI63-%' OR extra_json LIKE '%__AI_TEST_%'`],
    [`DELETE FROM operators WHERE name LIKE '__AI_TEST_%' OR phone LIKE '0196262%'`],
    [`DELETE FROM sessions WHERE token LIKE 'AI62-%' OR token LIKE 'AI63-%' OR kind LIKE 'LIVE_AUDIT_%' OR payload_json LIKE '%__AI_TEST_%'`]
  ];
}
async function liveAuditCleanup_(env) {
  for (const [sql] of liveAuditCleanSqls_()) await env.DB.prepare(sql).run();
}
async function liveAuditInsertOperator_(env, row) {
  await env.DB.prepare(`INSERT INTO operators (account_type, name, affiliation, phone, access, team_group, role, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .bind(row.type || 'JUDGE', row.name, row.affiliation || 'AI_TEST', normalizePhone(row.phone), row.access || '', row.team || '', row.role || '', nowIso(), nowIso()).run();
}
async function liveAuditInsertParticipant_(env, code, unit, name, extra={}) {
  await env.DB.prepare(`INSERT INTO participants (competition_code, name, affiliation, phone, unique_no, prelim_cup_no, main_cup_no, final_cup_no, cup_no, sample_no, team_name, team_no, extra_json, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .bind(code, name, 'AI_TEST', '01962629999', unit, unit, unit, unit, unit, unit, code === 'KTCC' ? name : '', code === 'KTCC' ? unit : '', JSON.stringify(extra || {}), nowIso(), nowIso()).run();
}
function liveAuditRow_(extra) { return { data: [], extraFields: Object.assign({}, extra || {}) }; }
function liveAuditPayload_(code, round, unit, actor, role, extra, mode='') {
  return {
    competitionCode: code,
    round,
    currentRound: round,
    unit,
    participantNo: unit,
    cupNo: code === 'KCR' ? unit : undefined,
    sampleNo: code === 'IKRC' ? unit : undefined,
    teamNo: code === 'KTCC' ? unit : undefined,
    judgeToken: actor && actor.judgeToken,
    judgeName: actor && actor.name,
    judgePhone: actor && actor.phone,
    judgeRole: role || (actor && actor.role) || '',
    role: role || (actor && actor.role) || '',
    team: (actor && actor.teamGroup) || '',
    teamGroup: (actor && actor.teamGroup) || '',
    mode,
    rows: [liveAuditRow_(extra)]
  };
}
function liveAuditKcacPayload_(round, unit, actor, rows, mode='') {
  return {
    competitionCode:'KCAC', round, currentRound:round, unit, participantNo:unit,
    judgeToken: actor && actor.judgeToken, judgeName: actor && actor.name, judgePhone: actor && actor.phone,
    judgeRole: actor && actor.role, role: actor && actor.role, mode,
    rows: rows.map(liveAuditRow_)
  };
}
function liveAuditPass_(checks, ok, name, detail={}) {
  checks.push({ ok: !!ok, name, detail });
  if (!ok) throw new Error(name + ' 실패: ' + JSON.stringify(detail));
}
function liveAuditFindRank_(ranking, unit, round='예선') {
  return (ranking || []).find(r => safeStr(r.unit) === safeStr(unit) && roundName_(r.round) === roundName_(round));
}
function liveAuditIdsForUnits_(reviewList, units) {
  const set = new Set((units || []).map(safeStr));
  return (reviewList || []).filter(x => set.has(safeStr(x.unit || x['참가자번호'] || x['컵번호'] || x['샘플번호'] || x['팀번호']))).map(x => x.rowIndex);
}
async function liveAuditLoginActors_(env) {
  const ops = [
    {key:'admin', type:'ADMIN', name:'__AI_TEST_ADMIN__', phone:'01962620000', access:'ALL', role:'관리자'},
    {key:'kcr', name:'__AI_TEST_KCR_JUDGE__', phone:'01962620001', access:'KCR', role:'센서리 심사위원'},
    {key:'kbc', name:'__AI_TEST_KBC_JUDGE__', phone:'01962620002', access:'KBC', role:'심사위원'},
    {key:'kcac', name:'__AI_TEST_KCAC_JUDGE__', phone:'01962620003', access:'KCAC', role:'헤드 심사위원'},
    {key:'moc', name:'__AI_TEST_MOC_STAFF__', phone:'01962620004', access:'MOC', role:'운영진', type:'STAFF'},
    {key:'ktcc', name:'__AI_TEST_KTCC_STAFF__', phone:'01962620005', access:'KTCC', role:'운영진', type:'STAFF'},
    {key:'ikrc', name:'__AI_TEST_IKRC_JUDGE__', phone:'01962620006', access:'IKRC', role:'센서리 심사위원'},
    {key:'ikrcHead', name:'__AI_TEST_IKRC_HEAD__', phone:'01962620007', access:'IKRC', role:'센서리 헤드 심사위원'},
    {key:'mobTech', name:'__AI_TEST_MOB_TECH__', phone:'01962620008', access:'MOB', role:'테크니컬 심사위원', team:'A'},
    {key:'mobS1', name:'__AI_TEST_MOB_S1__', phone:'01962620009', access:'MOB', role:'센서리1', team:'A'},
    {key:'mobS2', name:'__AI_TEST_MOB_S2__', phone:'01962620010', access:'MOB', role:'센서리2', team:'A'},
    {key:'mobS3', name:'__AI_TEST_MOB_S3__', phone:'01962620011', access:'MOB', role:'S3', team:'A'},
    {key:'mobHead', name:'__AI_TEST_MOB_HEAD__', phone:'01962620012', access:'MOB', role:'센서리 헤드 심사위원', team:'A'}
  ];
  for (const op of ops) await liveAuditInsertOperator_(env, op);
  const actors = {};
  for (const op of ops) {
    const logged = await judgeLogin(env, op.name, op.phone);
    if (!logged || !logged.success) throw new Error('테스트 계정 로그인 실패: ' + op.key + ' / ' + (logged && logged.message || ''));
    actors[op.key] = logged;
  }
  return actors;
}
async function liveAuditMarkComplete_(env, code, units, actor) {
  const review = await getReviewList(env, code, actor);
  if (!review || !review.success) throw new Error(code + ' 검수목록 조회 실패: ' + (review && review.message || ''));
  const ids = liveAuditIdsForUnits_(review.list, units);
  if (!ids.length) throw new Error(code + ' 검수완료 처리할 행이 없습니다: ' + units.join(','));
  const upd = await updateReviewStatus(env, code, ids, '검수완료', 'LIVE_AUDIT', actor);
  if (!upd || !upd.success) throw new Error(code + ' 검수완료 실패: ' + (upd && upd.message || ''));
  return { review, ids };
}
async function liveAuditRun(env, opts={}, request=null) {
  opts = opts && typeof opts === 'object' ? opts : {};
  const token = safeStr(opts.token || opts.auditToken || '');
  if (!timingSafeEqual_(token, liveAuditExpectedToken_(env))) return { success:false, message:'라이브 검수 토큰이 올바르지 않습니다.' };
  const startedAt = nowIso();
  const checks = [];
  const keepData = !!opts.keepData;
  const runId = 'AI62-' + Date.now().toString(36).toUpperCase();
  const units = {
    kcr1: runId + '-KCR-01', kcrCal: runId + '-KCR-CAL', kbc1: runId + '-KBC-01', kcac1: runId + '-KCAC-01',
    moc1: runId + '-MOC-01', ktcc1: runId + '-KTCC-01', ikrc1: runId + '-IKRC-01', ikrcHead: runId + '-IKRC-H', mob1: runId + '-MOB-01', mob2: runId + '-MOB-02'
  };
  let actors = null;
  try {
    await liveAuditCleanup_(env);
    actors = await liveAuditLoginActors_(env);
    for (const [code, arr] of Object.entries({ KCR:[units.kcr1, units.kcrCal], KBC:[units.kbc1], KCAC:[units.kcac1], MOC:[units.moc1], KTCC:[units.ktcc1], IKRC:[units.ikrc1, units.ikrcHead], MOB:[units.mob1, units.mob2] })) {
      for (const unit of arr) await liveAuditInsertParticipant_(env, code, unit, '__AI_TEST_' + code + '_' + unit, { runId });
    }

    // KCR: 평가 화면 내 켈리브레이션 모드 저장값이 순위에 섞이지 않는지 확인
    const kcrNormal = await submitScores(env, liveAuditPayload_('KCR', '예선', units.kcr1, actors.kcr, '센서리 심사위원', {
      '컵번호':units.kcr1, '프로세스':'Washed', 'Flavor(플레이버)':8, 'Aftertaste(애프터테이스트)':7, 'Acidity(산미)':6, 'Body(바디)':7, 'Sweetness(스윗니스) ×2':8, 'Overall(주관적 종합평가)':8, '총점':52
    }), null, request);
    liveAuditPass_(checks, kcrNormal.success, 'KCR 일반 평가 저장', kcrNormal);
    const kcrCal = await submitScores(env, liveAuditPayload_('KCR', '예선', units.kcrCal, actors.kcr, '센서리 심사위원', {
      '컵번호':units.kcrCal, '프로세스':'Calibration', 'Flavor(플레이버)':10, 'Aftertaste(애프터테이스트)':10, 'Acidity(산미)':10, 'Body(바디)':10, 'Sweetness(스윗니스) ×2':10, 'Overall(주관적 종합평가)':10, '총점':70
    }, '켈리브레이션'), null, request);
    liveAuditPass_(checks, kcrCal.success, 'KCR 켈리브레이션 평가 저장', kcrCal);
    await liveAuditMarkComplete_(env, 'KCR', [units.kcr1, units.kcrCal], actors.admin);
    const kcrRank = await getRanking(env, 'KCR', actors.admin);
    const kcrR1 = liveAuditFindRank_(kcrRank.ranking, units.kcr1);
    const kcrRC = liveAuditFindRank_(kcrRank.ranking, units.kcrCal);
    liveAuditPass_(checks, !!kcrR1 && Math.abs(Number(kcrR1.totalScore) - 52) < 0.001 && !kcrRC, 'KCR 총점/켈리브레이션 순위 제외', {rank:kcrR1, calibrationRank:kcrRC});

    // KBC: 시간감점 음수값이 감점으로 처리되는지 확인
    const kbcPayload = liveAuditPayload_('KBC', '예선', units.kbc1, actors.kbc, '심사위원', {
      '참가자번호':units.kbc1, 'Service Professionalism(서비스의 전문성)':5,
      'Espresso Taste & Design(맛과 설계) ×2':4, 'Espresso Clean Cup(클린컵)':4, 'Espresso Mouthfeel(마우스필)':4, 'Espresso Flavor(플레이버)':4,
      'Signature Taste & Design(맛과 설계) ×2':4, 'Signature Clean Cup(클린컵)':4, 'Signature Mouthfeel(마우스필)':4, 'Signature Flavor(플레이버)':4,
      'Machine & Equipment Professionalism(머신 및 기물 운용 전문성)':5, '시간감점':'-6', '경기시간':'07:05'
    });
    const kbcSubmit = await submitScores(env, kbcPayload, null, request);
    liveAuditPass_(checks, kbcSubmit.success, 'KBC 평가 저장', kbcSubmit);
    await liveAuditMarkComplete_(env, 'KBC', [units.kbc1], actors.admin);
    const kbcRank = await getRanking(env, 'KBC', actors.admin);
    const kbcR = liveAuditFindRank_(kbcRank.ranking, units.kbc1);
    liveAuditPass_(checks, !!kbcR && Math.abs(Number(kbcR.totalScore) - 43) < 0.001, 'KBC 음수 시간감점 보정/총점', {rank:kbcR});

    // KCAC: 2잔 합산 및 음수 감점 보정 확인
    const kcacSubmit = await submitScores(env, liveAuditKcacPayload_('예선', units.kcac1, actors.kcac, [
      {'참가자번호':units.kcac1, '우유종류':'매일우유', '잔용도':'FAST Rosetta', '예선 Pattern Completion(패턴 완성도)':5, '예선 Pattern Symmetry & Balance(대칭과 균형)':5, '예선 Surface Quality(표면 품질)':5, '예선 Position & Proportion(위치와 비율)':5, '예선 Pattern Definition(패턴 선명도)':5, '리프수감점':'-5'},
      {'참가자번호':units.kcac1, '우유종류':'어메이징 오트', '잔용도':'SLOW Rosetta', '예선 Pattern Completion(패턴 완성도)':4, '예선 Pattern Symmetry & Balance(대칭과 균형)':4, '예선 Surface Quality(표면 품질)':4, '예선 Position & Proportion(위치와 비율)':4, '예선 Pattern Definition(패턴 선명도)':4, '시간감점':'-3'}
    ]), null, request);
    liveAuditPass_(checks, kcacSubmit.success, 'KCAC 예선 2잔 평가 저장', kcacSubmit);
    await liveAuditMarkComplete_(env, 'KCAC', [units.kcac1], actors.admin);
    const kcacRank = await getRanking(env, 'KCAC', actors.admin);
    const kcacR = liveAuditFindRank_(kcacRank.ranking, units.kcac1);
    liveAuditPass_(checks, !!kcacR && Math.abs(Number(kcacR.totalScore) - 82) < 0.001, 'KCAC 2잔 합산/감점 보정', {rank:kcacR});

    // MOC / KTCC: 정답수·가산점·시간 기반 운영형 대회 확인
    const mocSubmit = await submitScores(env, liveAuditPayload_('MOC', '예선', units.moc1, actors.moc, '운영진', {'참가자번호':units.moc1, '정답수':4, '가산점':1, '종료시간':'04:58'}), null, request);
    liveAuditPass_(checks, mocSubmit.success, 'MOC 평가 저장', mocSubmit);
    await liveAuditMarkComplete_(env, 'MOC', [units.moc1], actors.admin);
    const mocR = liveAuditFindRank_((await getRanking(env, 'MOC', actors.admin)).ranking, units.moc1);
    liveAuditPass_(checks, !!mocR && Math.abs(Number(mocR.totalScore) - 5) < 0.001, 'MOC 정답수+가산점 총점', {rank:mocR});
    const ktccSubmit = await submitScores(env, liveAuditPayload_('KTCC', '예선', units.ktcc1, actors.ktcc, '운영진', {'팀번호':units.ktcc1, '팀명':'__AI_TEST_TEAM__', 'Section1 정답수':2, 'Section2 정답수':2, 'Section3 정답수':2, 'Section3 가산점':2, '종료시간':'07:33'}), null, request);
    liveAuditPass_(checks, ktccSubmit.success, 'KTCC 평가 저장', ktccSubmit);
    await liveAuditMarkComplete_(env, 'KTCC', [units.ktcc1], actors.admin);
    const ktccR = liveAuditFindRank_((await getRanking(env, 'KTCC', actors.admin)).ranking, units.ktcc1);
    liveAuditPass_(checks, !!ktccR && Math.abs(Number(ktccR.totalScore) - 8) < 0.001, 'KTCC 섹션 정답수+가산점 총점', {rank:ktccR});

    // IKRC: 헤드/켈리브레이션 제외 확인
    const ikrcNormal = await submitScores(env, liveAuditPayload_('IKRC', '예선', units.ikrc1, actors.ikrc, '센서리 심사위원', {'샘플번호':units.ikrc1, 'Flavor(플레이버) ×3':5, 'Clean Cup(클린컵) ×2':5, 'Sweetness(스윗니스) ×2':5, 'Acidity(산미)':5, 'Mouthfeel(마우스필) ×2':5}), null, request);
    liveAuditPass_(checks, ikrcNormal.success, 'IKRC 일반 평가 저장', ikrcNormal);
    const ikrcHead = await submitScores(env, liveAuditPayload_('IKRC', '예선', units.ikrcHead, actors.ikrcHead, '센서리 헤드 심사위원', {'샘플번호':units.ikrcHead, 'Flavor(플레이버) ×3':10, 'Clean Cup(클린컵) ×2':10, 'Sweetness(스윗니스) ×2':10, 'Acidity(산미)':10, 'Mouthfeel(마우스필) ×2':10}, '켈리브레이션'), null, request);
    liveAuditPass_(checks, ikrcHead.success, 'IKRC 헤드 켈리브레이션 저장', ikrcHead);
    await liveAuditMarkComplete_(env, 'IKRC', [units.ikrc1, units.ikrcHead], actors.admin);
    const ikrcRanking = await getRanking(env, 'IKRC', actors.admin);
    liveAuditPass_(checks, !!liveAuditFindRank_(ikrcRanking.ranking, units.ikrc1) && !liveAuditFindRank_(ikrcRanking.ranking, units.ikrcHead), 'IKRC 헤드/켈리브레이션 순위 제외', {ranking:ikrcRanking.ranking.filter(r => String(r.unit).includes(runId))});

    // MOB: 역할별 저장, 검수수정, 시간감점, 동점자 센서리 우선순위, 헤드 제외, 검수완료 숨김 확인
    const mobTech1 = await submitScores(env, liveAuditPayload_('MOB', '예선', units.mob1, actors.mobTech, '테크니컬 심사위원', {'참가자번호':units.mob1, 'Pre-Service Station(시연 전 작업대)':4, 'Service Station(시연 중 작업대)':5, 'Post-Service Station(시연 후 작업대)':2, '시간감점':'0', '경기시간':'10분 08.00초'}), null, request);
    liveAuditPass_(checks, mobTech1.success, 'MOB 테크니컬 평가 저장', mobTech1);
    const mobTech2 = await submitScores(env, liveAuditPayload_('MOB', '예선', units.mob2, actors.mobTech, 'T1', {'참가자번호':units.mob2, 'Pre-Service Station(시연 전 작업대)':4, 'Service Station(시연 중 작업대)':4, 'Post-Service Station(시연 후 작업대)':2, '시간감점':'6', '경기시간':'10.08'}), null, request);
    liveAuditPass_(checks, mobTech2.success, 'MOB 테크니컬 T1 역할 평가 저장', mobTech2);
    const sensoryRows = [
      [actors.mobS1, units.mob1, 4, 4, 4, 4, 4, 4], [actors.mobS2, units.mob1, 5, 4, 4, 4, 4, 4], [actors.mobS3, units.mob1, 3, 4, 4, 4, 4, 4],
      [actors.mobS1, units.mob2, 5, 5, 4, 4, 4, 3], [actors.mobS2, units.mob2, 5, 5, 4, 4, 4, 3], [actors.mobS3, units.mob2, 5, 5, 4, 4, 4, 3]
    ];
    for (const [actor, unit, sw, fl, ba, cc, mo, pr] of sensoryRows) {
      const sub = await submitScores(env, liveAuditPayload_('MOB', '예선', unit, actor, actor.role, {'참가자번호':unit, 'Sweetness(스윗니스)':sw, 'Flavor(플레이버)':fl, 'Balance(균형)':ba, 'Clean Cup(클린컵)':cc, 'Mouthfeel(질감)':mo, 'Professionalism(시연 전문성)':pr}), null, request);
      liveAuditPass_(checks, sub.success, 'MOB 센서리 평가 저장 ' + unit + '/' + actor.role, sub);
    }
    const mobHeadSub = await submitScores(env, liveAuditPayload_('MOB', '예선', units.mob1, actors.mobHead, '센서리 헤드 심사위원', {'참가자번호':units.mob1, 'Sweetness(스윗니스)':10, 'Flavor(플레이버)':10, 'Balance(균형)':10, 'Clean Cup(클린컵)':10, 'Mouthfeel(질감)':10, 'Professionalism(시연 전문성)':10}, '켈리브레이션'), null, request);
    liveAuditPass_(checks, mobHeadSub.success, 'MOB 헤드 켈리브레이션 저장', mobHeadSub);

    let mobReview = await getReviewList(env, 'MOB', actors.admin);
    liveAuditPass_(checks, mobReview.success && !mobReview.list.some(x => /헤드/i.test(String(x.role || x['역할'] || ''))), 'MOB 일반 검수목록 헤드 제외', {count: mobReview.list && mobReview.list.length});
    const mobTimePenaltyIndex = (mobReview.headers || []).indexOf('시간감점');
    const mobTech1Item = (mobReview.list || []).find(x => safeStr(x.unit) === units.mob1 && /테크|기술|T1|Technical/i.test(safeStr(x.role || x['역할'])));
    liveAuditPass_(checks, mobTimePenaltyIndex >= 0 && !!mobTech1Item, 'MOB 검수수정 대상/시간감점 컬럼 확인', {mobTimePenaltyIndex, mobTech1Item: mobTech1Item && mobTech1Item.rowIndex});
    const mobUpd = await updateReviewRow(env, 'MOB', mobTech1Item.rowIndex, {[mobTimePenaltyIndex]:'-6'}, '검수완료', 'LIVE_AUDIT', actors.admin);
    liveAuditPass_(checks, mobUpd.success, 'MOB 검수환경 시간감점 수정완료', mobUpd);
    await liveAuditMarkComplete_(env, 'MOB', [units.mob1, units.mob2], actors.admin);
    const mobCalBefore = await getMobCalibrationParticipantNumbers(env, 'A', '센서리 헤드 심사위원', actors.mobHead);
    liveAuditPass_(checks, Array.isArray(mobCalBefore) && mobCalBefore.some(x => safeStr(x.participantNo) === units.mob1), 'MOB 켈리브레이션 목록 표시', {mobCalBefore});
    const mobCalMark = await markMobCalibrationChecked(env, units.mob1, 'A', actors.mobHead.name, '센서리 헤드 심사위원', actors.mobHead);
    liveAuditPass_(checks, mobCalMark.success, 'MOB 켈리브레이션 검수완료 처리', mobCalMark);
    const mobCalAfter = await getMobCalibrationParticipantNumbers(env, 'A', '센서리 헤드 심사위원', actors.mobHead);
    liveAuditPass_(checks, Array.isArray(mobCalAfter) && !mobCalAfter.some(x => safeStr(x.participantNo) === units.mob1), 'MOB 검수완료 후 켈리브레이션 목록 숨김', {mobCalAfter});
    const mobRanking = await getRanking(env, 'MOB', actors.admin);
    const mobR1 = liveAuditFindRank_(mobRanking.ranking, units.mob1);
    const mobR2 = liveAuditFindRank_(mobRanking.ranking, units.mob2);
    liveAuditPass_(checks, !!mobR1 && !!mobR2 && Math.abs(Number(mobR1.totalScore) - 29) < 0.001 && Math.abs(Number(mobR2.totalScore) - 29) < 0.001 && Number(mobR2.rank) < Number(mobR1.rank), 'MOB 총점/동점자 센서리 우선순위/시간 파싱', {mobR1, mobR2});

    const finalReport = await getFinalReport(env, 'MOB', actors.admin);
    liveAuditPass_(checks, finalReport && finalReport.success && (finalReport.approvedRows || []).some(r => safeStr(r['식별번호']) === units.mob1 || safeStr(r['참가자번호']) === units.mob1), 'MOB 최종정리 검수완료 데이터 포함', {approvedRows:(finalReport.approvedRows || []).filter(r => JSON.stringify(r).includes(runId)).length});

    return { success:true, message:'Stage62 라이브 DB 자동검수 완료', runId, startedAt, finishedAt:nowIso(), keptData:keepData, checks, summary:{pass:checks.filter(x=>x.ok).length, fail:checks.filter(x=>!x.ok).length} };
  } catch (err) {
    return { success:false, message:'Stage62 라이브 DB 자동검수 중 오류가 발생했습니다.', runId, startedAt, failedAt:nowIso(), error:String(err && err.message || err), checks };
  } finally {
    if (!keepData) {
      try { await liveAuditCleanup_(env); } catch (e) { try { console.error('LIVE_AUDIT_CLEANUP_ERROR', e && e.stack ? e.stack : e); } catch (_) {} }
    }
  }
}


const LIVE_AUDIT_STEP_STATE_TOKEN_ = 'AI63-LIVE-AUDIT-STATE';
const LIVE_AUDIT_STEP_PHASES_ = ['SETUP', 'KCR', 'KBC', 'KCAC', 'MOC_KTCC', 'IKRC', 'MOB', 'FINAL'];
function liveAuditStepUnits_(runId) {
  return {
    kcr1: runId + '-KCR-01', kcrCal: runId + '-KCR-CAL', kbc1: runId + '-KBC-01', kcac1: runId + '-KCAC-01',
    moc1: runId + '-MOC-01', ktcc1: runId + '-KTCC-01', ikrc1: runId + '-IKRC-01', ikrcHead: runId + '-IKRC-H', mob1: runId + '-MOB-01', mob2: runId + '-MOB-02'
  };
}
async function liveAuditStepLoadState_(env) {
  const row = await env.DB.prepare('SELECT payload_json FROM sessions WHERE token=?').bind(LIVE_AUDIT_STEP_STATE_TOKEN_).first();
  return row && row.payload_json ? parseJson(row.payload_json, null) : null;
}
async function liveAuditStepSaveState_(env, state) {
  await env.DB.prepare('INSERT OR REPLACE INTO sessions (token, kind, payload_json, expires_at, created_at) VALUES (?, ?, ?, ?, ?)')
    .bind(LIVE_AUDIT_STEP_STATE_TOKEN_, 'LIVE_AUDIT_STATE', JSON.stringify(state || {}), '2035-12-31T23:59:59.000Z', nowIso()).run();
}
async function liveAuditStepEnsureOperator_(env, row) {
  const phone = normalizePhone(row.phone);
  const found = await env.DB.prepare('SELECT id FROM operators WHERE name=? AND phone=? LIMIT 1').bind(row.name, phone).first();
  if (!found) await liveAuditInsertOperator_(env, row);
}
async function liveAuditStepActors_(env) {
  const ops = [
    {key:'admin', type:'ADMIN', name:'__AI_TEST_ADMIN__', phone:'01962620000', access:'ALL', role:'관리자'},
    {key:'kcr', name:'__AI_TEST_KCR_JUDGE__', phone:'01962620001', access:'KCR', role:'센서리 심사위원'},
    {key:'kbc', name:'__AI_TEST_KBC_JUDGE__', phone:'01962620002', access:'KBC', role:'심사위원'},
  ...
  for (const op of ops) await liveAuditStepEnsureOperator_(env, op);
  const actors = {};
  for (const op of ops) {
    const logged = await judgeLogin(env, op.name, op.phone);
    if (!logged || !logged.success) throw new Error('테스트 계정 로그인 실패: ' + op.key + ' / ' + (logged && logged.message || ''));
...
async function liveAuditStepEnsureParticipant_(env, code, unit, runId) {
  const found = await env.DB.prepare('SELECT id FROM participants WHERE competition_code=? AND unique_no=? LIMIT 1').bind(code, unit).first();
  if (!found) await liveAuditInsertParticipant_(env, code, unit, '__AI_TEST_' + code + '_' + unit, { runId });
}
async function liveAuditStepEnsureBase_(env, state) {
  const actors = await liveAuditStepActors_(env);
  const units = liveAuditStepUnits_(state.runId);
  for (const [code, arr] of Object.entries({ KCR:[units.kcr1, units.kcrCal], KBC:[units.kbc1], KCAC:[units.kcac1], MOC:[units.moc1], KTCC:[units.ktcc1], IKRC:[units.ikrc1, units.ikrcHead], MOB:[units.mob1, units.mob2] })) {
    for (const unit of arr) await liveAuditStepEnsureParticipant_(env, code, unit, state.runId);
  }
  return { actors, units };
}
async function liveAuditRunStep(env, opts={}, request=null) {
  opts = opts && typeof opts === 'object' ? opts : {};
  const token = safeStr(opts.token || opts.auditToken || '');
  if (!timingSafeEqual_(token, liveAuditExpectedToken_(env))) return { success:false, message:'라이브 검수 토큰이 올바르지 않습니다.' };
  const reset = opts.reset === true || opts.reset === '1' || opts.reset === 1;
  const checks = [];
  let state = await liveAuditStepLoadState_(env);
  if (reset || !state || state.status === 'complete' || state.status === 'failed') {
    await liveAuditCleanup_(env);
    const runId = 'AI63-' + Date.now().toString(36).toUpperCase();
    state = { status:'running', runId, phaseIndex:0, startedAt:nowIso(), checks:[] };
    await liveAuditStepSaveState_(env, state);
  }
  const phase = LIVE_AUDIT_STEP_PHASES_[state.phaseIndex] || 'FINAL';
  const startedAt = nowIso();
  try {
    const ctx = await liveAuditStepEnsureBase_(env, state);
    const actors = ctx.actors, units = ctx.units;
    if (phase === 'SETUP') {
      liveAuditPass_(checks, !!actors.admin && !!units.kcr1, '라이브 검수 기본 계정/참가자 준비', { runId: state.runId });
    } else if (phase === 'KCR') {
      const kcrNormal = await submitScores(env, liveAuditPayload_('KCR', '예선', units.kcr1, actors.kcr, '센서리 심사위원', {
        '컵번호':units.kcr1, '프로세스':'Washed', 'Flavor(플레이버)':8, 'Aftertaste(애프터테이스트)':7, 'Acidity(산미)':6, 'Body(바디)':7, 'Sweetness(스윗니스) ×2':8, 'Overall(주관적 종합평가)':8, '총점':52
      }), null, request);
      ...
      const kbcSubmit = await submitScores(env, liveAuditPayload_('KBC', '예선', units.kbc1, actors.kbc, '심사위원', {
        '참가자번호':units.kbc1, 'Service Professionalism(서비스의 전문성)':5,
        'Espresso Taste & Design(맛과 설계) ×2':4, 'Espresso Clean Cup(클린컵)':4, 'Espresso Mouthfeel(마우스필)':4, 'Espresso Flavor(플레이버)':4,
        'Signature Taste & Design(맛과 설계) ×2':4, 'Signature Clean Cup(클린컵)':4, 'Signature Mouthfeel(마우스필)':4, 'Signature Flavor(플레이버)':4,
        'Machine & Equipment Professionalism(머신 및 기물 운용 전문성)':5, '시간감점':'-6', '경기시간':'07:05'
      ...
      await liveAuditMarkComplete_(env, 'IKRC', [units.ikrc1, units.ikrcHead], actors.admin);
      const ikrcRanking = await getRanking(env, 'IKRC', actors.admin);
      liveAuditPass_(checks, !!liveAuditFindRank_(ikrcRanking.ranking, units.ikrc1) && !liveAuditFindRank_(ikrcRanking.ranking, units.ikrcHead), 'IKRC 헤드/켈리브레이션 순위 제외', {ranking:ikrcRanking.ranking.filter(r => String(r.unit).includes(state.runId))});
    } else if (phase === 'MOB') {
      const mobTech1 = await submitScores(env, liveAuditPayload_('MOB', '예선', units.mob1, actors.mobTech, '테크니컬 심사위원', {'참가자번호':units.mob1, 'Pre-Service Station(시연 전 작업대)':4, 'Service Station(시연 중 작업대)':5, 'Post-Service Station(시연 후 작업대)':2, '시간감점':'0', '경기시간':'10분 08.00초'}), null, request);
      liveAuditPass_(checks, mobTech1.success, 'MOB 테크니컬 평가 저장', mobTech1);
      const mobTech2 = await submitScores(env, liveAuditPayload_('MOB', '예선', units.mob2, actors.mobTech, 'T1', {'참가자번호':units.mob2, 'Pre-Service Station(시연 전 작업대)':4, 'Service Station(시연 중 작업대)':4, 'Post-Service Station(시연 후 작업대)':2, '시간감점':'6', '경기시간':'10.08'}), null, request);
      ...
      liveAuditPass_(checks, !!mobR1 && !!mobR2 && Math.abs(Number(mobR1.totalScore) - 29) < 0.001 && Math.abs(Number(mobR2.totalScore) - 29) < 0.001 && Number(mobR2.rank) < Number(mobR1.rank), 'MOB 총점/동점자 센서리 우선순위/시간 파싱', {mobR1, mobR2});
      const finalReport = await getFinalReport(env, 'MOB', actors.admin);
      liveAuditPass_(checks, finalReport && finalReport.success && (finalReport.approvedRows || []).some(r => safeStr(r['식별번호']) === units.mob1 || safeStr(r['참가자번호']) === units.mob1), 'MOB 최종정리 검수완료 데이터 포함', {approvedRows:(finalReport.approvedRows || []).filter(r => JSON.stringify(r).includes(state.runId)).length});
    } else if (phase === 'FINAL') {
      const passCount = (state.checks || []).filter(x => x.ok).length;
      liveAuditPass_(checks, passCount >= 20, '전체 단계 누적 검수 통과 수 확인', { passCount });
      await liveAuditCleanup_(env);
      state.status = 'complete';
    }
    state.checks = (state.checks || []).concat(checks.map(c => Object.assign({ phase }, c)));
    if (phase !== 'FINAL') state.phaseIndex += 1;
    if (phase === 'FINAL') state.finishedAt = nowIso();
    state.lastPhase = phase;
    state.lastUpdatedAt = nowIso();
    await liveAuditStepSaveState_(env, state);
    const complete = state.status === 'complete';
    return { success:true, message: complete ? 'Stage63 단계형 라이브 DB 자동검수 전체 완료' : 'Stage63 단계형 라이브 DB 자동검수 단계 완료 - 같은 URL을 다시 호출하면 다음 단계를 진행합니다.', runId:state.runId, phase, completed:complete, nextPhase:complete ? null : LIVE_AUDIT_STEP_PHASES_[state.phaseIndex], checks, summary:{ totalPass:(state.checks||[]).filter(x=>x.ok).length, totalFail:(state.checks||[]).filter(x=>!x.ok).length } };
  } catch (err) {
    state.status = 'failed';
    state.lastPhase = phase;
    ...
    state.checks = (state.checks || []).concat(checks.map(c => Object.assign({ phase }, c)));
    await liveAuditStepSaveState_(env, state);
    return { success:false, message:'Stage63 단계형 라이브 DB 자동검수 중 오류가 발생했습니다.', runId:state.runId, phase, startedAt, failedAt:nowIso(), error:state.error, checks, summary:{ totalPass:(state.checks||[]).filter(x=>x.ok).length, totalFail:(state.checks||[]).filter(x=>!x.ok).length } };
  }
}
